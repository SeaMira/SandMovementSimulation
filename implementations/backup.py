...
def sand_move():
    ...
    @window.event
    def on_draw():
        global transfer_rate, repose_angle, max_steps, sand_transport_block_count, seconds
        ...
        # Slider para la tasa de transferencia (0.0 a 1.0)
        changed, transfer_rate = imgui.slider_float(
            "Velocidad Caída", transfer_rate, 0.0, 1.0
        )
        
        # Slider para ángulo de reposo (20 a 45 grados)
        changed_angle, repose_angle = imgui.slider_float(
            "Angulo Reposo", repose_angle, 20.0, 45.0
        )
        
        
        # Slider para coef. de sticky mask Kb (0.0 a 1.0)
        changed_angle, kb = imgui.slider_float(
            "Kb", kb, 0.0, 1.0
        )
        ...
        # Slider para pasos de viento (R_s)
        changed_rs, max_steps = imgui.slider_int("Pasos (R_s)", max_steps, 1, 50)
        
        # Cantidad de bloques a mover
        ...
        if imgui.button("Reiniciar Simulación"):
             # Aquí podrías llamar a una función que llene los SSBO con ceros o ruido de nuevo
             pass

        imgui.end()

        # Renderizar la UI sobre todo lo demás
        imgui.render()
